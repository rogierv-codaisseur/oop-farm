...
test('The Wheat class calculates the proper yield', () => {
  const wheat = new Wheat(100);
  expect(wheat.getYieldInKg()).toBeCloseTo(674.4, 1);
});

test('The Wheat class calculates the proper yield value', () => {
  const wheat = new Wheat(100);
  expect(wheat.getYieldInEuros()).toBeCloseTo(1011.6, 1);
});

test('The Wheat class calculates the proper costs value', () => {
  const wheat = new Wheat(123);
  expect(wheat.getCosts()).toBeCloseTo(41820);
});
...
test('The Sugarcane class calculates the proper yield value', () => {
...
test('The Sugarcane class calculates the proper costs value', () => {
  const sugarcane = new Sugarcane(0);
